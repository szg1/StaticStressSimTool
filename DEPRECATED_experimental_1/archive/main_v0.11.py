...
def load_materials_db():
    """
    Loads materials from json file.
    """
    global MATERIALS_DB, SELECTED_MATERIAL
    try:
        with open('materials.json', 'r') as f:
            MATERIALS_DB = json.load(f)
        # Default to PLA if available
        if "Bambu PLA Basic" in MATERIALS_DB:
            SELECTED_MATERIAL = MATERIALS_DB["Bambu PLA Basic"]
            SELECTED_MATERIAL["name"] = "Bambu PLA Basic"
...
def get_material_selection_gui():
    """
    Opens a dialog to select a material from the database.
    """
    if not MATERIALS_DB:
        print("No materials database loaded.")
        return None

    # Format the list for the prompt
    mat_list = list(MATERIALS_DB.keys())
    mat_str = "Available Materials:\\n"
    ...
    if sys.platform == 'darwin':
        try:
            script = f"""
            display dialog "{prompt}" default answer "{default_val}" with title "Select Material" buttons {{"Cancel", "Select"}} default button "Select"
        ...
        try:
            ...
            root.withdraw()
            root.update()
            # Tkinter dialogs might not handle newlines well in prompt, simplifying
            simple_prompt = "Available:\n" + "\n".join([f"{i+1}. {n}" for i,n in enumerate(mat_list)])
            input_str = simpledialog.askstring("Select Material", 
                                             simple_prompt + "\n\nEnter number or name:", 
    ...
    if input_str.isdigit():
        idx = int(input_str) - 1
        if 0 <= idx < len(mat_list):
            selected_key = mat_list[idx]
    else:
        # Check if name match (case insensitive)
...
def material_callback(vis):
    """
    Triggered by 'M' key.
    Opens dialog to select material.
    """
    global SELECTED_MATERIAL
    print("\n--- Material Selection ---")
    
    mat_key = get_material_selection_gui()
    if mat_key:
        SELECTED_MATERIAL = MATERIALS_DB[mat_key]
        SELECTED_MATERIAL["name"] = mat_key
        print(f"Material set to: {mat_key}")
        print(f"  Density: {SELECTED_MATERIAL['density_g_cm3']} g/cm3")
        print(f"  Young's Modulus: {SELECTED_MATERIAL['youngs_modulus_mpa']} MPa")
        return True
    
    print("Material selection cancelled.")
    return False
...
def main():
    ...
    vis.register_key_callback(77, material_callback) # 'M'
