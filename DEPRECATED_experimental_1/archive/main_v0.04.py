...
def detect_hole_locations(mesh):
    ...
    IMPROVED METHOD: 
    1. Finds vertical walls to locate the hole center (XY).
    2. Scans inside that location for a 'shelf' (counterbore floor) to set the Z depth.
    """
    ...
    # We look for surfaces that are:
    # a) Upward facing (Normal Z > 0.9)
    # b) Strictly BELOW the top surface (e.g., > 1mm deep)
    # This identifies the floor of the counterbore.
    ...
    # Filter for walls in the top section
    ...
    for i in range(max_label + 1):
        ...
        # Calculate Centroid XY
        center_xy = np.mean(cluster_pts, axis=0)
        
        # --- DETECT Z-DEPTH (Counterbore Shelf) ---
        target_z = max_z # Default to top if no shelf found
        
        if len(shelf_pts) > 0:
            # Look for shelf points within a radius of this hole center
            # M10 head is ~18mm dia, so radius ~9mm. We check 12mm to be safe.
            dx = shelf_pts[:, 0] - center_xy[0]
            dy = shelf_pts[:, 1] - center_xy[1]
            dist_sq = dx*dx + dy*dy
            
            # Points within ~12mm radius (144mm^2)
            nearby_shelf_indices = np.where(dist_sq < 144.0)[0]
            
            if len(nearby_shelf_indices) > 0:
                nearby_z = shelf_pts[nearby_shelf_indices, 2]
                # Take the median Z of the shelf points to be robust against noise
                target_z = np.median(nearby_z)
                print(f"     Feature {i}: Found internal shelf at Z={target_z:.2f} (Depth: {max_z - target_z:.2f}mm)")
    ...
    if len(candidates) > 0:
        ...
        for next_cand in candidates[1:]:
            ...
            if dist < 6.0: 
                ...
                # Average the group positions (including Z)
...
def fit_screw_to_hole(screw, base, hole_index):
    ...
    # target_pos[2] is now the Z-height of the SHELF, not the top surface
