...
def perform_carving_tensor(mesh, layer_h, nozzle_w, walls, infill_pct, infill_type):
    ...
    # 2. Define Query Grid
    # High resolution for Marching Cubes
    res = nozzle_w / 2.0
    ...
    max_dim_voxels = 400 
    if np.max(dims) / res > max_dim_voxels:
        res = np.max(dims) / max_dim_voxels
        print(f"  -> Model too large. Adjusting resolution to {res:.2f}mm")
